...
The Hilbert--Schmidt criterion suggests a local question: does the Hellinger
distance created by a small matrix perturbation measure its Frobenius norm,
with constants that survive as the dimension grows? The upper bound follows
by integrating the Fisher length of a path. The lower bound requires a way
to detect every direction of motion. We find it by first asking what a
rotation does to two independent coordinates.

\subsection{Detecting a rotation}

Characteristic functions let us detect dependence with bounded tests. To
remove the separate responses of the two marginals, we center each
exponential before multiplying. For independent copies \(X_1,X_2\) with
\(\E X=0\) and \(\E|X|<\infty\), write \(\phi(u)=\E e^{iuX}\) and set
\[
...
\]
Before rotation, independence gives
\begin{equation}
...
\end{equation}
Also \(|W_{\xi,\eta}|\le4\). Thus the test remains integrable after mixing,
regardless of the tails. Its derivative at zero rotation answers the
question suggested by Gaussian invariance: can a non-Gaussian law escape
every one of these tests? The following infinitesimal form of Gaussian
independence rigidity \cite{kac1939characterization,kagan1973characterization}
...
To recover the law from a vanishing response, fix one frequency and regard
the other as the variable of an ODE. Suppose $R\equiv0$ and choose $\eta_0\ne0$ with
...
The response has a simple geometric interpretation. The two entries of the
weighted Fourier jet
\[
 u\longmapsto\bigl(u\phi(u),\phi'(u)\bigr)\in\mathbb C^2
\]
form a curve whose two-point determinants are
\begin{equation}
 \det\!\begin{pmatrix}
 \xi\phi(\xi)&\phi'(\xi)\\
 \eta\phi(\eta)&\phi'(\eta)
 ...
 =-R(\xi,\eta).
\end{equation}
For a Gaussian this curve lies in a line; for every non-Gaussian it spans
two directions. This is exactly the freedom we will need to separate the
two ordered entries of a matrix tangent. The weight \(u\) comes from
differentiating the exponential under rotation. It makes Gaussianity a
linear dependence relation, and fixing one nonzero frequency proves that
relation globally, even across zeros of \(\phi\).

\subsection{The matrix Fisher operator}

The rotation calculation identifies the exceptional law. To measure the
size of a general infinitesimal mixing, we now compute its Fisher metric
\cite{amari2000methods}. There are three natural motions: changing individual
scales, mixing symmetrically, and rotating. The calculation below assigns
one eigenvalue to each, and shows exactly where the Gaussian metric loses
a direction. Write \(g=\sqrt f\) and assume
\begin{equation}
 \begin{gathered}
...
\end{equation}
The location score \(s=-f'/f=-2g'/g\) satisfies
\begin{equation}
...
\subsection{Following square-root densities}

The rotational eigenvalue \(J-1\) is the part of the Fisher metric that
covariance misses. Together with the scale and symmetric eigenvalues, it
measures every matrix tangent when the law is non-Gaussian. To turn this
infinitesimal calculation into a comparison of measures, we work with
square-root densities: their ordinary \(L^2\) distance is Hellinger distance,
and linear changes of variables act unitarily. This is the
quadratic-mean viewpoint of \cite[Section~7.2]{vandervaart1998asymptotic};
see also \cite{bogachev1997differentiable,bell1985quasi}.

For $A\in\GL_n(\R)$ define a unitary operator on
$L^2(\R^n,dx)$ by
\[
 (U_Av)(y)=|\det A|^{-1/2}v(A^{-1}y),
 \qquad
 g^{\otimes n}(x)=\prod_{i=1}^n g(x_i).
\]
Thus $U_Ag^{\otimes n}$ is the square root of the Lebesgue density of
$A_\#\mu_n$.  For invertible $A,B$,
\[
 h(A_\#\mu_n,B_\#\mu_n)=\|U_Ag^{\otimes n}-U_Bg^{\otimes n}\|_2.
\]
For a fixed real matrix $A$, the generator of $U_{\exp(tA)}$ is
\begin{equation}
 G_Av=-\tfrac12\operatorname{tr}(A)v-(Ax)\cdot\nabla v,
 \qquad G_Ag^{\otimes n}=\tfrac12S_Ag^{\otimes n}.
...
$\GL_n(\R)$, then $t\mapsto U_{A_t}g^{\otimes n}$ is continuously
differentiable in $L^2$ and
\begin{equation}
 \begin{aligned}
 \frac d{dt}U_{A_t}g^{\otimes n}
 &=U_{A_t}G_{A_t^{-1}\dot A_t}g^{\otimes n},\\
 \left\|\frac d{dt}U_{A_t}g^{\otimes n}\right\|_2
...
We now return to a non-Gaussian \(f\) and seek a lower bound. Integrating the
speed of a path gives an upper bound on the distance travelled; a lower
bound must also prevent cancellation along the path. We do this by pairing
the displacement with a test chosen for its direction.

The score \(S_A\) suggests the right pairing, but following the score itself
would introduce derivatives and moments beyond our assumptions. The
Fourier jet gives a smoother choice. Each off-diagonal score term is
\(s(X_i)X_j\). We choose bounded Fourier sums \(p,q\) dual to \(s,x\), so
that \(p(X_i)q(X_j)\) detects the ordered entry \(A_{ij}\) and cancels the
reversed entry. A third Fourier sum detects changes of scale.
...
For
\begin{equation}
 H_A(X_1,\ldots,X_n):=\sum_i A_{ii}r(X_i)
...
\begin{equation}
 ...
 \qquad \|H_A\|_{L^2(\mu_n)}\le C\|A\|_F,
...
The dual pairing is exact at the identity. To use it along a path, we must
control how fast the test itself moves. The apparent obstacle is the number
of terms in \(H_AS_B\), which grows like the fourth power of the dimension.
Most cross terms vanish by centering. Grouping the others by their shared
coordinates leaves a fixed number of patterns, each controlled by a
Frobenius contraction. This is where independence makes the estimate
...
\eqref{eq:fourier-functions}.  There is a constant $C$,
depending on these functions and on $J,\kappa$, such that
\begin{equation}
 \|G_B(H_Ag^{\otimes n})\|_{L^2(dx)}
 \le C\|A\|_F\|B\|_F
...
with \(\E X^2=1\) and the weighted Sobolev regularity of \(g^{\otimes n}\), puts
\(H_Ag^{\otimes n}\) in the domain of \(G_B\).  The product rule gives
\begin{equation}
 G_B(H_Ag^{\otimes n})
 =\left\{\tfrac12H_AS_B-(Bx)\cdot\nabla H_A\right\}g^{\otimes n}.
...
\(D_{a}(r)+Q_{A_0}(p,q)\) and
\(D_{b}(d)+Q_{B_0}(s,x)\), where
$a=(A_{ii})_i$, $b=(B_{ii})_i$,
$A_0=A-\diag A$, and
$B_0=B-\diag B$.
...
give \((B_{ii}M_{ij})\), \(M^*b\), \((M_{ij}B_{ij})_{ij}\), and their
...
We can now keep the first-order response from cancelling. Pairing with
\(H_A\) gives a quadratic main term, while the generator bound makes the
error cubic. Choosing one sufficiently small radius therefore works in
every dimension.
...
\begin{align*}
 \langle H_Ag^{\otimes n},(U_{\exp A}-I)g^{\otimes n}\rangle
 &=\int_0^1
 \langle U_{\exp(-tA)}(H_Ag^{\otimes n}),G_Ag^{\otimes n}\rangle\,dt.
\end{align*}
The value obtained by replacing $U_{\exp(-tA)}(H_Ag^{\otimes n})$ with
$H_Ag^{\otimes n}$ is $\frac12\E H_AS_A=\frac12\|A\|_F^2$.  Moreover,
...
\[
 \left|\langle H_Ag^{\otimes n},(U_{\exp A}-I)g^{\otimes n}\rangle
                       -\tfrac12\|A\|_F^2\right|
 \le\frac{C\sqrt{C_f}}4\|A\|_F^3.
\]
...
Near the identity, Hellinger distance now measures Frobenius distance. The
same is true near every allowed coordinate symmetry, since these preserve
the law. The global question is whether matrices far from all such
symmetries could nevertheless change the law by arbitrarily little.
The infinite-dimensional classification will rule this out: stacking such
matrices in independent blocks would contradict Hilbert--Schmidt necessity.
We state the resulting comparison here and prove it in
...
that, for every \(n\) and every \(T\in\GL_n(\R)\) whose singular
values lie in \([a,b]\),
\begin{equation}
 c(1\wedge d_n(T)^2)
 \le h^2(\mu_n,T_\#\mu_n)
 \le C(1\wedge d_n(T)^2).
