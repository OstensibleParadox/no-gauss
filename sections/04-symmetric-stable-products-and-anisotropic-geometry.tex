...
\cref{lem:measurable-row-series-realization}; it is an assumption on the row-series realization, not a claim
...
compatible stable row-series realization shows that \(T\) and its row inverse map this
...
\]
The positive stable estimates, for \(y>0\),
\[
...
\(((T^{-1})^*)_\#\Lambda_k\), so finite-dimensional uniqueness identifies
...
The local input is the following dimension-free estimate,
...
\end{equation}
To verify this, note that \(\Theta_u(\mathbf X)\) has mean zero and a variance independent
of \(u\).
...
Oddness then proves \eqref{eq:stable-directional-detection}.
...
gives uniform invariance under finite tail sign changes:
\begin{equation}
...
Thus \(T\in\mathcal P^{\pm}\), while every member of \(\mathcal P^{\pm}\) plainly
preserves \(\mu_\alpha\).  This proves the assertion about the exact linear
