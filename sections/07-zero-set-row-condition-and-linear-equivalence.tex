\section{Zero-Set Row Condition and Linear Equivalence}
\label{sec:zero-set-row-condition-and-linear-equivalence}

The positivity assumption in the main theorem has one precise job: it
removes a separate zero-set obstruction from the finite-marginal likelihood
ratios.
Once the density has zeros, the Hilbert--Schmidt condition remains the right
metric accumulation condition, but it is no longer the whole story.
The missing datum is the zero-set row condition.
Each row of the operator, and for equivalence each row of the inverse, must
land in the set where the one-dimensional density is positive almost surely.

This section gives the zero-set row-condition replacement first.
Arbitrary zero sets lead to an exact zero-set row condition; bounded
coordinate laws force signed permutations with only the scalar reflection
freedom allowed by the one-dimensional measure class; centered half-line laws
force ordinary permutations.
We then give examples showing why the a.e.-positive-density classification
does not extend unchanged and how boundary behavior changes the summability
threshold.
...
The zero-set row-condition theorem still uses the same Fourier/Fisher mechanism for
the metric part of the classification.
The logarithm-free Fourier characterization in \cref{lem:fourier-ode}, and
its one-rotation Hellinger lower bound, already require no density.
...
The first replacement theorem is the main zero-set row-condition form of the
classification.
The Hilbert--Schmidt alternative remains the metric part of the answer.
The additional zero-set row condition is purely about null sets: no output coordinate may place
positive probability in the zero set of the original density, and mutual
equivalence requires the same condition after applying the inverse operator.
The condition concerns the actual zero set of the density, rather than its
closed support.
...
Thus density zeros do not introduce an additional global likelihood-ratio
limit beyond the operator criterion.
They introduce the zero-set row condition.
Every nonzero row sum has an absolutely continuous scalar law: condition
on all but one input with a nonzero coefficient.  Thus the zero-set row conditions
...
For a density with gaps, the rows of $T$ and $T^{-1}$ must avoid those
gaps almost surely.  The contraction in
\cref{sec:zero-set-row-condition-counterexample} below satisfies the forward
zero-set row condition but fails the inverse one, which is exactly why it gives
strict absolute continuity rather than equivalence.

\subsection{Boundary behavior of the density and summability criteria}
\label{sec:boundary-behavior-and-summability}

The zero-set row condition and the projected Hellinger energy measure different
phenomena.  The former decides whether the image law is absolutely
continuous in either direction; Hellinger loss decides whether overlap
accumulates to zero.
The two block-rotation examples below separate these effects.

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{p{0.28\textwidth} p{0.27\textwidth} p{0.32\textwidth}}
\toprule
\textbf{Boundary type} & \textbf{One-block loss} & \textbf{What it controls}\\
\midrule
Hard boundary, uniform square & linear in \(|\theta|\) & accumulated
Hellinger loss; absolute continuity still fails for every nonzero
boundary-crossing block\\
Smooth vanishing boundary & quadratic in \(\theta\) & the Fourier/Fisher
metric threshold; the zero-set row condition remains separate\\
\bottomrule
\end{tabular}
\end{center}
...
For bounded and one-sided supports, the zero-set row condition has much stronger
consequences than it first suggests.
Conditional expectations of finite row sums see the boundary of the support.
This turns the boundary restriction into an \(\ell^1\)-type row constraint, and
the inverse relations then force the operator to be monomial.
The classification below therefore needs no density and no Fisher
information.
...
This is the main bounded-support and half-line payoff.
...
The zero-set row condition is not a cosmetic addition to the
Hilbert--Schmidt criterion.
It separates positive Hellinger overlap from genuine two-sided absolute
continuity even for a rank-one perturbation.
...
For reference, the boundary examples above use the following product
criterion.
It separates absolute continuity within each pair of factors from
accumulation of Hellinger loss, and is not part of the linear i.i.d.
...
For reference, there is also a purely measure-theoretic fallback when one
does not want to use the operator classification.
For densities satisfying \eqref{eq:zero-set-row-assumptions},
\cref{thm:linear-equivalence-group} gives the sharper linear answer.
The following finite-marginal criterion applies without Sobolev assumptions
and without any dichotomy.
...
\(\prod_{i\leq n}f(x_i)\).
Thus \eqref{eq:two-sided-finite-marginal-ui} is an exact criterion for
bounded invertible linear mixing even in this case.
Under \eqref{eq:zero-set-row-assumptions},
\cref{thm:linear-equivalence-group} identifies the linear equivalence
group by a Hilbert--Schmidt condition and forward and inverse zero-set row conditions for the
operator and its inverse.  This includes unbounded densities with gaps.
For bounded and one-sided laws,
\cref{thm:bounded-and-half-line-classification} further reduces the group to
