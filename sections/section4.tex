...
This section constructs the required measurable versions and then identifies
the global measure class through likelihood martingales and a dense
translation group.
...
\]
Data processing makes \(E_F\) monotone under inclusion.  Because the initial
segments \(\{1,\ldots,n\}\) are cofinal among finite subsets,
\(\sup_F E_F=\sup_nE_n\).  Thus the energy criterion does not depend on a
privileged enumeration even though initial segments are convenient for the
martingale notation.
