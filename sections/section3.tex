...
After realizing row series on a common measurable carrier, we prove
Hilbert--Schmidt sufficiency by approximation and necessity by combining tail
rigidity with conjugated coordinate swaps.
...
\subsection{Rational translation invariance and the equivalence--singularity dichotomy}

The relevant translation theory identifies shifts preserving product and
coordinatewise measure classes
\cite{shepp1965translate,hora1991quasi,sadasue2008quasi,arutyunyan2015spaces,honda2015approximations}.
Here \(\ell^2\)-quasi-invariance and rational-translation ergodicity specialize
the equivalence--singularity dichotomy of \cite{okazaki1985dichotomy} to each
bounded invertible orbit.
...
The finite-marginal martingale converges only to the density of the absolutely
continuous part of \(\nu_T\); the equivalence--singularity dichotomy upgrades positive
limiting affinity to equivalence.  See
\cite{lecam1986asymptotic,mandrekar2018lebesgue} for the likelihood-ratio and
Lebesgue-decomposition background, and \cite{kitada1989absolute} for the
distinct problem of independent additive perturbations.
...
\end{equation}
Thus \(xg'\in L^2\) is not required.  Cutoff integration by parts still gives
\(\E s(X)=0\) and \(\E[Xs(X)]=1\), while a skew-symmetric \(A\) has only
off-diagonal score terms and
\begin{equation}
 \E S_A^2=(J-1)\|A\|_F^2.
\end{equation}
...
The next two lemmas use only a centered, variance-one, non-Gaussian coordinate
law: a row whose linear combination converges in law to \(X\) must approach an
allowed signed coordinate.
...
Covariance compactness and unit-row rigidity match remote rows and columns
with distinct allowed coordinates, but do not sum the errors.  If their sum
diverges, a remote packet can be swapped with better-matched auxiliary
coordinates.  After conjugation this gives \(W=PT^{-1}ST\), whose
Hilbert--Schmidt size is comparable to the packet error.  Local coercivity
detects \(W\), whereas absolute continuity makes the remote output swap
asymptotically invisible.
...
To compare the norms, freeze the auxiliary matches by defining
\[
...
\]
while the commutator satisfies
\[
...
To globalize local coercivity, stack a hypothetical sequence of
finite-dimensional counterexamples with summable Hellinger energies into one
block-diagonal operator.  Kakutani gives equivalence, so the classification
makes this operator Hilbert--Schmidt-close to an allowed signed coordinate permutation;
that permutation must eventually preserve the blocks, a contradiction.
