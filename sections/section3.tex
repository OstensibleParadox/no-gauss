...
an \(L^2\) equivalence class, but such classes cannot be composed pointwise:
...
The composition identity is the delicate point: separate almost-sure
realizations do not by themselves compose.  For finitely supported \(b\), the
finite sum of output coordinates equals
...
all rational finite-coordinate translations has, by a.e. positivity of the
...
case, write \(\nu_T=\ell_T\mu\).  Quasi-invariance of both measures implies
that \(\{\ell_T>0\}\) is invariant modulo \(\mu\) under \(\mathcal A\).
Its \(\mu\)-measure is positive because \(\int \ell_T\,d\mu=1\), and
...
of the a.e.-positive density of \(AU\).  This mixture is positive
...
on cylinder sets gives \(\ell_{T,\infty}\mu\leq\nu_T\), first on the cylinder
algebra and then on the generated sigma-field.  Conversely, if
\((\nu_T)_a=\ell_{\mathrm{ac}}\mu\), then
\(\E_\mu[\ell_{\mathrm{ac}}\mid\mathcal F_n]\leq \ell_{T,n}\), so martingale
convergence gives \(\ell_{\mathrm{ac}}\leq \ell_{T,\infty}\).  The two inequalities identify
the limit.
Since \(\E_\mu(\sqrt{\ell_{T,n}})^2=1\), the square roots are uniformly
integrable in \(L^1(\mu)\).  They thus converge in \(L^1\) to
\(\sqrt{\ell_{T,\infty}}\).  Conditional Jensen's inequality gives the
...
They have a nonnegative limit of norm one; denote its square by
$\widetilde\ell_T$.
Cauchy--Schwarz also gives
\begin{equation}
 \|\ell_{T_m}-\ell_{T_n}\|_1
 \le2\|\sqrt{\ell_{T_m}}-\sqrt{\ell_{T_n}}\|_2,
 \qquad \ell_{T_n}\longrightarrow \widetilde\ell_T
       \quad\text{in }L^1(\mu).
 \label{eq:approximant-l1-cauchy}
\end{equation}
Let $\mathbf X=(X_j)_{j\ge1}$ have law $\mu$.
...
Bounded continuous cylinder tests identify $\widetilde\ell_T\mu$ with
$\nu_T$.
...
\cref{lem:measurable-row-series-realization} requires only the centered finite
second moment.
On the common measurable carrier for the required countable group it gives
\[
 \mu=(\Phi_T)_\#\nu_{T^{-1}}
       \ll(\Phi_T)_\#\mu=\nu_T.
\]
...
Change of variables on the same common measurable carrier proves the cocycle
and inverse identities.
Finally, $\nu_Q=\mu$ and $T=(TQ^{-1})Q$ give the assertion about the exact
linear stabilizer.
\end{proof}
...
\end{lemma}
\begin{proof}
The compactness mechanism separates retained coefficients from an escaping
Gaussian component.  After reordering and passing to a subsequence, the
coefficients converge coordinatewise; the squared mass escaping
through coefficients that tend individually to zero becomes an independent
Gaussian because only the quadratic term of \(\phi\) survives.  If the
resulting law is again \(\rho\) and no limiting coefficient has modulus one,
iterating the same smoothing identity makes every iterated coefficient product
geometrically small and forces \(\phi(t)=e^{-t^2/2}\).  Non-Gaussianity
therefore leaves only one possible fixed point: a single allowed signed
coordinate.  We now make these three steps precise.
...
\]
is at most
$t^2\omega_\rho(|t|/\sqrt{m+1})$, uniformly in $n$.
Fix any $t$.
...
\end{equation}
Equivalently, the limiting law is that of $\sum_j a_jX_j$ plus an
independent centered Gaussian of variance $1-\tau_a$.
...
$m_a:=\max_j|a_j|$ exists, since $(a_j)\in\ell^2$, and satisfies
$0<m_a<1$.  For
...
Thus some
$a_j=\varsigma\in\{-1,1\}$; all the remaining coefficients and
the independent Gaussian summand vanish.  The limiting identity then says
$\Law(\varsigma X)=\rho$, so the sign preserves $\rho$.
...
\end{lemma}
\begin{proof}
For the self-adjoint covariance defect \(TT^*-I\), compactness will follow once
the operator norms of its remote tail compressions tend to zero.  Absolute
continuity compares \(\mu\) and \(\nu_T\) on bounded tail tests, whereas the
quadratic forms of the covariance defect are unbounded.  The real issue is
therefore uniform integrability of the squares of all \(\ell^2\) linear sums
with bounded coefficient norm.  The truncation below supplies exactly this
bridge using a second moment only.
...
zero in $L^2$.
Therefore \cref{lem:unit-row-rigidity} gives indices $\iota(i)$ and signs
$\varsigma_i\in\{-1,1\}$ preserving $\rho$, outside a finite initial set, with
\begin{equation}
 \widehat e_i:=\varsigma_i e_{\iota(i)},\qquad
 \|T^*e_i-\widehat e_i\|_2^2\longrightarrow0.
 \label{eq:qualitative-tail-matching}
 \end{equation}
The indices $\iota(i)$ are distinct sufficiently far out.
Indeed, if
$\iota(i)=\iota(j)$ for two such rows, then
...
Hence we can fix $N_0$ so that $\iota$ is injective on $i>N_0$.

There is also column approximation along this matching.
Compactness
of $T^*T-I$ and the fact that $\iota(i)$ leaves every finite set imply
\[
 \|T\widehat e_i\|_2^2\longrightarrow1.
\]
Since $\langle T\widehat e_i,e_i\rangle
=\langle \widehat e_i,T^*e_i\rangle\to1$,
we obtain $\|T\widehat e_i-e_i\|_2^2\to0$.  Consequently
\begin{equation}
...
We prove that $\sum_i\operatorname{err}_i<\infty$ on this tail.
By
\cref{lem:local-hs-coercivity}, there are
$c_{\mathrm{HS}},\delta_{\mathrm{HS}}>0$ such
that
\begin{equation}
...
\end{equation}
Suppose instead that $\sum_i\operatorname{err}_i=\infty$.
Choose $\delta>0$ small enough that
$3\|T^{-1}\|_{\mathrm{op}}\delta<\delta_{\mathrm{HS}}$.
In any sufficiently remote tail, where
$\operatorname{err}_i\leq\delta^2$, choose a finite set $E$ satisfying
\begin{equation}
...
Since $\operatorname{err}_i\to0$, these auxiliary indices may be chosen with
an arbitrarily small value of $\sum_{k\in G}\operatorname{err}_k$, while
$E$ remains fixed.
...
\cref{lem:measurable-row-series-realization} justifies this identity on a
common measurable carrier.

We now verify the norm comparison.  The auxiliary operator \(T_0\) freezes the
nearly exact matches in \(G\), so the remaining commutator isolates precisely
the row-and-column error carried by \(E\).  Obtain \(T_0\) from \(T\) by
replacing each auxiliary row and column with
...
the displayed sum of squared errors tends to
$\sum_{i\in E}\operatorname{err}_i$.
Also,
\[
...
Therefore $\sum_i\operatorname{err}_i<\infty$.
...
Let \(c_{\mathrm{loc}},C_{\mathrm{loc}},\delta_{\mathrm{loc}}\) be the
constants in \cref{prop:local-fisher-hellinger}, decreasing
\(\delta_{\mathrm{loc}}\) so that \(0<\delta_{\mathrm{loc}}<1\).
Given \(T\), choose a minimizer
\(Q_n\in\mathcal P_n\) in \eqref{eq:distance-stabilizer}.  Since
\((Q_n)_\#\mu_n=\mu_n\),
\[
 h(\mu_n,T_\#\mu_n)
 ...
 \|TQ_n^{-1}-I\|_F=d_n(T).
\]
When \(d_n(T)\le\delta_{\mathrm{loc}}\), the local theorem supplies the
two-sided linear bounds for \(h\).  When
\(d_n(T)>\delta_{\mathrm{loc}}\),
\eqref{eq:uniform-separation} with
\(\varepsilon=\delta_{\mathrm{loc}}\) supplies the lower bound, while
\(h^2\le2\) supplies the upper bound.  Choose $c>0$ no larger than
$c_{\mathrm{loc}}^2$ and
the positive infimum in \eqref{eq:uniform-separation} with
$\varepsilon=\delta_{\mathrm{loc}}$, and take
\[
