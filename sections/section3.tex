...
\subsection{Making row sums composable}

We will compare a mixing with its inverse and with conjugated coordinate
exchanges. These operations require actual maps on the product space. A
sample almost surely lies outside \(\ell^2\), even though every row sum is
well defined in \(L^2\). The way to make the maps composable is to choose
the subsequence defining each sum from its coefficients alone. The same
choice then works before and after a bounded linear mixing. This realization
of Hilbert-space operators on the product space goes back to
\cite{shimomura1977linear}. Throughout this construction,
\(\E X=0\) and \(\E X^2=1\).
...
\[
 ...
 \leq C_\lambda2^{-4k}.
\]
Chebyshev's inequality and Borel--Cantelli give convergence along this
subsequence \(\lambda\)-almost surely and in \(L^2(\lambda)\).  This
deterministic choice works for every law satisfying
\eqref{eq:covariance-domination}.  Set
...
\subsection{Why overlap implies equivalence}
...
\subsection{Reading the full law through finite marginals}

Every finite output marginal has a positive density. Singularity can
therefore emerge only as more coordinates are observed. The likelihood
ratios form a martingale, and the limiting affinity measures the overlap
that survives these observations. By the preceding dichotomy, any positive
overlap gives equivalence. The likelihood interpretation is standard
\cite{lecam1986asymptotic,mandrekar2018lebesgue}; here it lets us compare
dependent outputs, whereas independent additive perturbations can be
studied through convolution \cite{kitada1989absolute}.
...
and \(\E X=0\), \(\E X^2=1\).
For \(T\in\GL(H)\), every finite output marginal
...
\end{equation}
Then
\(\Aff(\mu_n,(\pi_n)_\#\nu_T)=\E_\mu\sqrt{\ell_{T,n}}\), and \(E_n(T)\) is
as in \eqref{eq:projected-hellinger-energy}; moreover,
\((\ell_{T,n})_{n\geq1}\) is a nonnegative mean-one \(\mu\)-martingale.
We have
\begin{equation}
...
martingale limit is
\(\ell_{T,\infty}=d(\nu_T)_{\mathrm{ac}}/d\mu\), the density of the absolutely continuous
part of \(\nu_T\).  Indeed, Fatou on cylinder sets gives
\(\ell_{T,\infty}\mu\leq\nu_T\); conversely, if
\((\nu_T)_{\mathrm{ac}}=\ell_{\mathrm{ac}}\mu\), then
...
\[
  \lim_n\Aff\bigl(\mu_n,(\pi_n)_\#\nu_T\bigr)
      ...
      =\Aff(\mu,\nu_T).
\]
The dichotomy in \cref{lem:equivalence-singularity-dichotomy} now proves
...
Since initial segments are cofinal among finite coordinate sets, Hellinger
data processing also makes the criterion independent of their enumeration
\cite{liese2006divergences}.
...
\cref{lem:measurable-row-series-realization} supplies the common measurable carrier
