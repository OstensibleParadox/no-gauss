...
$a_j=\varsigma\in\{\pm1\}$.  All other coefficients and the Gaussian
...
\[
 ...
 \|TQ_n^{-1}-I\|_F=d_n(T).
\]
For \(d_n(T)\le\delta_{\mathrm{loc}}\), the local theorem gives both bounds.
For \(d_n(T)>\delta_{\mathrm{loc}}\),
\eqref{eq:uniform-separation} gives the lower bound and \(h^2\le2\) the upper
bound.  Taking the smaller
lower constant and
\(C=\max\{C_{\mathrm{loc}}^2,2\delta_{\mathrm{loc}}^{-2}\}\) proves
