...
The local estimate answers how much a small perturbation changes the law.
The classification asks the converse question: how can equivalence of laws
force a general operator to be a small perturbation of a coordinate symmetry
in the tail? Absolute continuity will identify the individual axes. The
decisive step is then to compare errors on one group of coordinates with
negligible errors on another, using exchanges that preserve the product law.

\subsection{Sufficiency of Hilbert--Schmidt perturbations}

For a Hilbert--Schmidt perturbation, the finite-dimensional path bound is
already enough. Truncate the matrix to finitely many coordinates. The
remaining Hilbert--Schmidt norm tends to zero, so the square-root likelihoods
are Cauchy and converge to a density for the mixed law. Applying the same
argument to the inverse gives the reverse absolute continuity.

\begin{proposition}[Hilbert--Schmidt sufficiency and likelihood-ratio convergence]
\label{prop:hilbert-schmidt-sufficiency}
Assume \eqref{eq:fisher-regularity} and let
...
The inverse approximants likewise satisfy
\(\ell_{T_n^{-1}}\to\ell_{T^{-1}}\) in \(L^1(\mu)\).
If $Q$ is an exact linear stabilizer of $\mu$, then \(\nu_T\sim\mu\)
whenever \(TQ^{-1}-I\in\mathcal S_2\) and \(T\) is invertible.
\end{proposition}
...
\]
Applying the same argument gives $\nu_{T^{-1}}\ll\mu$. To deduce
reverse absolute continuity, we compose the row maps. Choose their versions
from the coefficients alone, so the choice works before and after mixing.
For \(a\in H\), take \(n_k(a)\ge k\) with
\(\sum_{j>n_k(a)}|a_j|^2\le2^{-4k}\), and define \(\widehat L_a\) as the
limit of these partial sums, or zero when it does not exist. For any law
\(\lambda\) with
\begin{equation}
 \int\left|\sum_j a_jx_j\right|^2\,d\lambda(x)
       \le C_\lambda\|a\|_2^2
       \quad(a\text{ finitely supported}),
 \label{eq:covariance-domination}
\end{equation}
the squared \(L^2\) error is at most \(C_\lambda2^{-4k}\), so
Borel--Cantelli identifies \(\widehat L_a\) with the row sum almost surely.
Set \(\Phi_T(x)_i=\widehat L_{T^*e_i}(x)\). Finite linear combinations
of these rows give \(\widehat L_{T^*b}\); hence the output law has bound
\(C_\lambda\|T\|_{\mathrm{op}}^2\). Taking \(L^2\) limits in the next
row sum gives \(\widehat L_b\circ\Phi_T=\widehat L_{T^*b}\) and therefore
\begin{equation}
 \Phi_S\circ\Phi_T=\Phi_{ST}\quad\lambda\text{-almost surely}
       \qquad(S,T\in\mathcal B(H)).
 \label{eq:measurable-composition}
\end{equation}
For a fixed countable operator group, intersecting the composition sets
and their group preimages gives an invariant set on which the maps are
mutual inverses. The second-moment bound makes this set full under
\(\mu\) and all its orbit laws. This is the row-series realization used
throughout \cite{shimomura1977linear}. In particular,
\[
 \mu=(\Phi_T)_\#\nu_{T^{-1}}
       \ll(\Phi_T)_\#\mu=\nu_T.
\]
...
Finally, $\nu_Q=\mu$ and $T=(TQ^{-1})Q$ give the assertion about exact
stabilizers.
\end{proof}

\subsubsection{Rotations and the scale score}

An orthogonal path has a skew-symmetric tangent, whose diagonal vanishes.
The scale score therefore disappears from both the path length and the
Fourier pairing. The assumptions reduce to
\begin{equation}
...
\end{equation}
Cutoff integration by parts gives \(\E s(X)=0\), \(\E[Xs(X)]=1\), and,
for skew-symmetric \(A\),
\begin{equation}
 \E S_A^2=(J-1)\|A\|_F^2.
\end{equation}
Only \(x_j\partial_ig^{\otimes n}\) with \(i\ne j\) occur in the generator formula,
which follows by radial cutoff and mollification commuting with the orthogonal
one-parameter group.
...
\begin{equation}
 c\|V-I\|_F\le h(V_\#\mu_n,\mu_n)
                   \le C\|V-I\|_F
 \quad(V\in\mathrm O(n),\ \|V-I\|_F\le\delta).
...
Truncation does not preserve orthogonality. We restore it by taking the
polar factor of $T_n=I+P_n(U-I)P_n$:
\[
 V_n=T_n(T_n^*T_n)^{-1/2}.
\]
...
We now start from equivalence of laws, with no prescribed coordinate
symmetry. Before locating that symmetry, we pass the local comparison to
Hilbert--Schmidt operators. This will let us apply the finite-dimensional
estimate to the conjugated exchanges at the end of the argument.
...
\begin{proof}
Let \(K_n=P_nKP_n\).  Since the untouched tail has affinity one,
\eqref{eq:local-matrix-hellinger} applies to \(I+K_n\), while
...
For the orthogonal assertion, choose the radius in this lemma to be half the
finite-dimensional radius in \eqref{eq:orthogonal-local-hellinger}.  The polar
approximants from \cref{prop:orthogonal-sufficiency} satisfy
\[
 h(\nu_{V_n},\mu)\longrightarrow h(\nu_{I+K},\mu),
 \qquad
 \|V_n-I\|_{\mathcal S_2}\longrightarrow\|K\|_{\mathcal S_2},
\]
Thus all sufficiently large approximants lie within the finite-dimensional
radius, and \eqref{eq:orthogonal-local-hellinger} passes to the limit.
\end{proof}

\subsubsection{Finding the coordinate axes}

Why should a row whose output resembles \(X\) concentrate on a single
coordinate? If its coefficients are all small, their sum is approximately
Gaussian. More generally, large coefficients survive in a subsequential
limit and the remaining small coefficients contribute a Gaussian summand.
A non-Gaussian law can reproduce itself in this way only when one surviving
coefficient is an allowed sign. The next two lemmas prove this using just
\(\E X=0\), \(\E X^2=1\), and non-Gaussianity.
...
then
\begin{equation}
 \inf_{Q\in\mathcal P}\|a^{(n)}-Qe_1\|_2\longrightarrow0.
 \label{eq:unit-row-qualitative}
\end{equation}
\end{lemma}
\begin{proof}
To separate surviving coefficients from a diffuse tail, reorder each row
by decreasing absolute value; this preserves the law of the
sum and gives $|a_j^{(n)}|\le j^{-1/2}$.  From any subsequence, extract a
further one with $a_j^{(n)}\to a_j$ for every $j$.  Fatou gives
\[
 \|a\|_2^2=\sum_j a_j^2\leq1.
\]
...
\begin{equation}
 e^{-(1-\|a\|_2^2)t^2/2}\prod_j\phi(a_jt).
 \label{eq:unit-row-gaussian-summand}
\end{equation}
Thus the limit is $\sum_j a_jX_j$ plus an independent
$\mathcal N(0,1-\|a\|_2^2)$ variable.

The limiting law is \(\rho\). If every surviving coefficient is smaller
than one, substituting this representation into itself repeatedly makes
all coefficients small. The resulting law must then be Gaussian.
To see this, suppose that no $|a_j|$ is one. If all $a_j$ vanish,
\eqref{eq:unit-row-gaussian-summand} already makes $X$ Gaussian.
Otherwise $\|a\|_\infty=\max_j|a_j|$ exists and lies in $(0,1)$.  For
...
\begin{equation}
 \phi(t)=e^{-(1-\|a\|_2^{2k})t^2/2}
       \prod_{\mathbf j\in\N^k}\phi(tb_{\mathbf j}),
 \label{eq:smoothing-iteration}
\end{equation}
where
\[
 \sum_{\mathbf j} b_{\mathbf j}^2=\|a\|_2^{2k},
 \qquad \sup_{\mathbf j}|b_{\mathbf j}|\leq \|a\|_\infty^k.
\]
...
\[
 |\phi(t)-e^{-t^2/2}|
 \leq t^2\|a\|_2^{2k}\omega_\rho(|t|\|a\|_\infty^k)\longrightarrow0.
\]
This contradicts non-Gaussianity.  Hence some
$a_j=\varepsilon\in\{\pm1\}$.  All other coefficients and the Gaussian
summand vanish, and the limiting identity gives $\Law(\varepsilon X)=\rho$.
...
produces an allowed $\varepsilon e_j$ with
\[
...
An absolutely continuous change of a product law can be approximated by
changing only a finite prefix. Beyond that prefix, coordinate permutations
are almost invisible and the marginals are almost unchanged. We will use
both consequences of this observation.
...
The marginal conclusion identifies the limiting row laws. To use unit-row
rigidity, we also need their lengths. In fact, absolute continuity controls
the covariance on the entire remote subspace, giving a compact covariance
defect. The only obstacle is that covariance is unbounded; truncating the
input variables will justify passing from closeness of laws to closeness
of second moments.
...
$N$, put
$\widehat L_a(x)^2=(\sum_i a_ix_i)^2$.  Under $\mu$, these are squares from
...
uniformly integrable under both laws.  The bounded statistic $\widehat L_a^2\wedge L$
depends only on tail coordinates, so independence gives
\[
 \left|\E_\nu(\widehat L_a^2\wedge L)-\E_\mu(\widehat L_a^2\wedge L)\right|
 \leq L\|\ell-\ell_N\|_1.
\]
Letting first $N\to\infty$ and then $L\to\infty$, uniformly in $a$, gives
\[
 \sup_{\substack{a\in c_{00},\ \|a\|_2=1\\ P_Na=0}}
...
The axes are now identifiable, but the errors could still tend to zero too
slowly to be square summable. To exclude this, compare a finite group of
imperfect matches with an equal number of much better matches farther out.
Exchange the two groups at both input and output. Exact matches would make
the input exchange followed by \(T\) agree with \(T\) followed by the output
exchange. The discrepancy therefore measures the matching error. Composing
with the inverse maps turns it into a perturbation of the identity, where
the local Hellinger lower bound applies. Absolute continuity, meanwhile,
makes the remote output exchange
invisible. The next proof makes these two comparisons quantitative.

\begin{lemma}[Hilbert--Schmidt necessity under a second-moment hypothesis]
\label{lem:hilbert-schmidt-necessity}
Assume \eqref{eq:main-assumptions}.
If $T\in\GL(H)$ and $\nu_T\sim\mu$, then
\begin{equation}
 T-Q\in\mathcal S_2
...
signs $\varepsilon_i$ such that
\begin{equation}
 \widehat e_i:=\varepsilon_i e_{\iota(i)},\qquad
...
\begin{equation}
 r_i:=\|T^*e_i-\widehat e_i\|_2^2
       +\|T\widehat e_i-e_i\|_2^2\longrightarrow0.
\end{equation}

We have matched both the row and its corresponding column; together their
errors will control the discrepancy between the exchanges. We claim that
$\sum_{i>N_0}r_i<\infty$. Choose the local
constants in \cref{lem:local-hs-coercivity} so that
\begin{equation}
 h(\mu,\nu_W)\geq c\|W-I\|_{\mathcal S_2}
...
In an arbitrarily remote tail where $r_i\le\delta^2$, choose
a finite set $E$ satisfying
\begin{equation}
 \delta^2\leq\sum_{i\in E}r_i\leq2\delta^2.
 \label{eq:active-tail-error}
\end{equation}
Pair each $i\in E$ with a distinct later index $k_i$ such that, for
$G=\{k_i:i\in E\}$, the sum $\sum_{k\in G}r_k$ is
arbitrarily small.
...
\end{equation}
The composition law \eqref{eq:measurable-composition} justifies the
change of variables.

The norm comparison becomes transparent if the auxiliary matches are exact.
We can make this change at arbitrarily small Hilbert--Schmidt cost. Define
\[
 T_0^*e_k=\widehat e_k,\qquad T_0\widehat e_k=e_k\qquad(k\in G).
\]
On the complementary block let \(T_0\) agree with \(T\).  Orthogonal block
decomposition
gives
\begin{equation}
...
\]
while the discrepancy satisfies
\[
...
\end{equation}
As $\sum_{k\in G}r_k\to0$, the first displayed error tends
to $\sum_{i\in E}r_i$, while
\[
...
$h(\mu,\nu_W)\geq c\delta/(2\|T\|_{\mathrm{op}})$.
...
$\sum_{i>N_0}r_i<\infty$.

It remains to complete the tail matching to a permutation of all coordinates.
The number of unused inputs must equal the number of unmatched outputs;
this follows from the Fredholm index. Define $Q_0$ by the matched rows
$\widehat e_i$ after the fixed initial cutoff
...
The preceding arguments classify equivalence by the distance from a
coordinate symmetry. To finish the theorem, we identify the alternative
as singularity and recover the same distinction from finite observations.

\paragraph{Why overlap implies equivalence.}

Two arbitrary measures can overlap without being equivalent. For the
positive-density products considered here, translations rule out this
intermediate possibility. Both the original and the mixed law admit
\(\ell^2\) shifts, and neither can be split into nontrivial pieces invariant
under those shifts. Their common part must therefore be all or nothing.
This is the mechanism of Okazaki's dichotomy
\cite{okazaki1985dichotomy}. The product translation theory is developed in
\cite{shepp1965translate,hora1991quasi,sadasue2008quasi,arutyunyan2015spaces,honda2015approximations};
we give the argument needed here.

\begin{lemma}[Equivalence--singularity dichotomy on every bounded invertible orbit]
\label{lem:equivalence-singularity-dichotomy}
For every \(T\in\GL(H)\),
\begin{equation}
  \nu_T\sim\mu\quad\text{or}\quad\nu_T\perp\mu.
\end{equation}
The assumptions are \(f>0\) a.e., \(\sqrt f\in W^{1,2}(\R)\),
\(\E X=0\), and \(\E X^2=1\).
\end{lemma}
...
Write \(\nu_T=\nu_T^{\mathrm{ac}}+\nu_T^{\mathrm{s}}\) for its Lebesgue decomposition relative to
\(\mu\).  Choose a Borel \(\mu\)-null set \(N_0\) supporting
\(\nu_T^{\mathrm{s}}\), and let \(N=\bigcup_{a\in\mathcal A}(N_0+a)\).
Quasi-invariance of \(\mu\) makes \(N\) a \(\mu\)-null set;
it is exactly \(\mathcal A\)-invariant and still supports \(\nu_T^{\mathrm{s}}\).
Thus \(\nu_T(N)=\nu_T^{\mathrm{s}}(\R^{\N})\in\{0,1\}\) by ergodicity.  Mass one gives
...
\paragraph{Reading the full law through finite marginals.}

The Hellinger energy records how much overlap survives as more outputs are
observed. This uses their joint distributions and therefore applies to
dependent coordinates as well. We collect the affinity limit and the
likelihood identities used below \cite{lecam1986asymptotic,mandrekar2018lebesgue}.

\begin{lemma}[Affinity and likelihood ratios through finite marginals]
\label{lem:finite-marginal-likelihood-ratios}
For any probability laws \(\lambda,\nu\) on \(\R^{\N}\),
\begin{equation}
 \Aff((\pi_n)_\#\lambda,(\pi_n)_\#\nu)
       \downarrow\Aff(\lambda,\nu).
 \label{eq:general-marginal-affinity-limit}
\end{equation}
Under the assumptions of \cref{lem:equivalence-singularity-dichotomy},
every \(T\in\GL(H)\) has positive finite-dimensional output densities and
\begin{equation}
 \nu_T\sim\mu\quad\Longleftrightarrow\quad\sup_nE_n(T)<\infty;
 \qquad E_n(T)\uparrow\infty\quad\text{otherwise}.
 \label{eq:marginal-affinity-limit}
\end{equation}
If \(\nu_T=\ell_T\mu\sim\mu\), its marginal likelihood ratios satisfy,
with \(\mathcal F_n=\sigma(x_1,\ldots,x_n)\),
\begin{equation}
 \begin{aligned}
 \ell_{T,n}
 &=\frac{d[(\pi_n)_\#\nu_T]}{d\mu_n}(\pi_n x)
   =\E_\mu[\ell_T\mid\mathcal F_n]
       \longrightarrow\ell_T &&\text{in }L^1(\mu),\\
 \ell_{T,n}^{-1}
 &=\E_{\nu_T}[\ell_T^{-1}\mid\mathcal F_n]
       \longrightarrow\ell_T^{-1} &&\text{in }L^1(\nu_T).
 \end{aligned}
\end{equation}
Both martingales are uniformly integrable, and
\(\Aff(\mu_n,(\pi_n)_\#\nu_T)=\E_\mu\sqrt{\ell_{T,n}}\).
\end{lemma}
\begin{proof}
Take \(m=(\lambda+\nu)/2\), write \(u=d\lambda/dm\), \(v=d\nu/dm\),
and let \(u_n,v_n\) be their conditional expectations on \(\mathcal F_n\).
Since \(u,v\le2\), martingale convergence gives
\[
 \Aff((\pi_n)_\#\lambda,(\pi_n)_\#\nu)
 =\int\sqrt{u_nv_n}\,dm\longrightarrow\int\sqrt{uv}\,dm.
\]
Conditional Cauchy--Schwarz gives monotonicity. The limit is positive
exactly when the laws overlap, so the preceding dichotomy gives
\eqref{eq:marginal-affinity-limit}.

The first \(n\) rows of \(T\) contain an invertible input minor \(A\).
Their output has the form \(AU+W\), where \(U\sim\mu_n\) is independent
of \(W\). Its density is a mixture of translates of the positive density
of \(AU\), proving finite-dimensional equivalence. Under full equivalence,
the two likelihood identities follow by integrating over
\(\mathcal F_n\); conditional-expectation convergence proves the rest.
\end{proof}

\paragraph{Exact linear stabilizer.}
Exact invariance requires only the second moment.
...
$\nu_T\sim\mu$.  For \(K=TQ^{-1}-I\), the same proposition shows that the
likelihood ratios for \(I+P_nKP_n\) converge to \(d\nu_T/d\mu\) in
\(L^1(\mu)\), and their square roots converge in \(L^2(\mu)\).
\Cref{lem:equivalence-singularity-dichotomy,lem:finite-marginal-likelihood-ratios} give the
projected-energy characterization, the singular alternative, and two-sided
uniform integrability.  The exact linear stabilizer was proved above.
\end{proof}
...
\cref{lem:equivalence-singularity-dichotomy}.
\end{proof}

\subsection{Returning to the global finite-dimensional bound}
\label{sec:globalization-proof}

The infinite-dimensional theorem also answers the question left open at the
end of Section~2. Suppose matrices could stay a fixed Frobenius distance
from every coordinate symmetry while their Hellinger distances tended to
zero. Choose them with summable Hellinger losses and stack them in
independent blocks. Kakutani would give equivalence, whereas our
classification would force those same blocks to approach coordinate
symmetries. This contradiction gives uniform separation in every dimension.
...
\begin{equation}
 \inf_{\substack{n\ge1,\ A\in\GL_n(\R)\\
                 a^2I\le A^*A\le b^2I,\ d_n(A)\ge\varepsilon}}
 h^2(\mu_n,A_\#\mu_n)>0.
 \label{eq:uniform-separation}
\end{equation}
If this fails, choose matrices \(A_k\in\GL_{n_k}(\R)\) in the displayed class
whose Hellinger distances tend to zero.  Since
\[
 -2\log\Aff(\mu_{n_k},(A_k)_\#\mu_{n_k})
 =-2\log\left(1-\tfrac12
 h^2(\mu_{n_k},(A_k)_\#\mu_{n_k})\right),
 \]
...
\begin{equation}
 A-Q\in\mathcal S_2(H).
\end{equation}

Let \(\Pi_k\) be the orthogonal projection onto the \(k\)-th output block.
The orthogonality of these blocks gives
\[
 \sum_k\|\Pi_k(A-Q)\|_{\mathcal S_2}^2=\|A-Q\|_{\mathcal S_2}^2<\infty,
 \qquad\text{hence}\qquad
 \|\Pi_k(A-Q)\|_{\mathcal S_2}\longrightarrow0.
 \]
...
$\|\Pi_k(A-Q)\|_{\mathcal S_2}^2<a^2+1$.  Thus $Q^*$ preserves every sufficiently
...
Let $c,C_{\mathrm{loc}},\delta$ be the local
constants, with $0<\delta<1$.  Choose a minimizer
...
\[
 ...
 \|TQ_n^{-1}-I\|_F=d_n(T).
\]
For \(d_n(T)\le\delta\), the local theorem gives both bounds.
For \(d_n(T)>\delta\),
\eqref{eq:uniform-separation} gives the lower bound and \(h^2\le2\) the upper
bound.  Taking the smaller
lower constant and
\(C=\max\{C_{\mathrm{loc}}^2,2\delta^{-2}\}\) proves
\eqref{eq:global-coercivity}.
\end{proof}
