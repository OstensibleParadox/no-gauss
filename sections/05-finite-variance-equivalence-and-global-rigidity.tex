...
We now prove the measure-class classification.
Sufficiency constructs a convergent infinite-dimensional likelihood ratio from
finite-coordinate Hilbert--Schmidt approximants.
Necessity proves that any non-Hilbert--Schmidt tail deviation leaves a
Hellinger trace that cannot be hidden by bounded invertible linear mixing.
The global finite-dimensional coercivity theorem is proved at the end,
after the classification supplies the required compactness principle.

\subsection{Sufficiency of Hilbert--Schmidt perturbations}
\label{sec:sufficiency}

We first prove the constructive direction of the classification.
A Hilbert--Schmidt perturbation can be approximated by finite-coordinate
changes whose Hellinger increments are summable.
The proposition turns this finite-dimensional Cauchy property into an actual
infinite-dimensional likelihood ratio.
...
The finite-dimensional lower bound must also apply to perturbations whose
rows are not finitely supported.
The next lemma transfers local coercivity from coordinate blocks to genuine
Hilbert--Schmidt operators by approximation and likelihood-ratio convergence.
It is the quantitative tool used later to turn many small tail mismatches
...
The preceding row rigidity gives only qualitative matching of far-out rows
with signed coordinates.
To obtain a Hilbert--Schmidt conclusion, those row errors must be summable.
The proof forces summability by swapping carefully chosen tail coordinates:
if the errors were not summable, the swaps would create a small finite-rank
perturbation that local coercivity can detect, while tail invariance makes
it asymptotically invisible.
...
\cref{lem:measurable-row-series-realization} justifies this identity on a common
carrier.
...
For an i.i.d. product whose non-Gaussian coordinate law is centered with
variance one, the exact linear stabilizer is $\mathcal P$.
...
The exact linear stabilizer is identified in \cref{prop:exact-linear-stabilizer}.
...
Let $f>0$ almost everywhere be a non-Gaussian density centered with
variance one, and assume only $\sqrt f\in W^{1,2}(\R)$ in addition
...
We now prove the global finite-dimensional statement announced in
\cref{sec:global-coercivity}.
This final argument is deliberately placed after the main theorem.
It uses the infinite-dimensional classification to rule out a sequence of
finite-dimensional almost-symmetries that stay a fixed distance from the
finite-dimensional exact linear stabilizer.
In this way the Hilbert--Schmidt theorem supplies the compactness missing
from a purely finite-dimensional proof.
