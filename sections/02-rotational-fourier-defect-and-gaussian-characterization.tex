...
This section rules out the only way a non-Gaussian law could imitate the
Gaussian rotation symmetry at first order.
If every infinitesimal two-coordinate rotation had zero rotational Fourier
defect, the coordinate law would have to be Gaussian.
This is the identification input later used by the dimension-free
coercivity argument.
The product of centered complex exponentials defining this defect is bounded.
...
The following lemma is the point at which the rotational Fourier defect becomes a
rigidity tool.
Later arguments use its contrapositive: if the coordinate law is not
Gaussian, then some rotational direction has a nonzero first-order response.
The formulation stays at the level of characteristic functions, so it avoids
choosing a branch of \(\log\phi\) and remains valid even when \(\phi\) has
zeros.
...
This reformulation is included because the finite-dimensional proof needs a
finite set of test frequencies, not merely an abstract nonvanishing defect.
The determinant viewpoint identifies the rank-two obstruction used to build
the dual moments in \cref{sec:fourier-dual-moments}.
...
Equation \eqref{eq:centered-exponential-product-variance} is the variance
estimate behind the finite real Fourier sums used below.
...
In particular, the product of centered complex exponentials does not require
a fourth moment, symmetry, or a density.
...
the product of centered complex exponentials after rotating two independent
copies of $Y$.
...
The Laplace law centered with variance one has rotational Fourier defect $R(1,2)=-4/27$.
