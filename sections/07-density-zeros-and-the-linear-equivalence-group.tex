...
Finally, the proof of \cref{lem:tail-rigidity} uses absolute continuity
only through \cref{lem:tail-symmetry-uniform,lem:compact-covariance-rigidity},
which assume \(\nu_T\ll\mu\).
...
\subsection{The linear equivalence group for arbitrary density zero sets}
\label{sec:linear-equivalence-group}

The Hilbert--Schmidt condition and the scalar positivity set together give
an exact operator criterion, including for densities with gaps and with
both tails unbounded.  The required condition on the rows concerns the
...
\end{equation}
Consequently the linear equivalence group is
\begin{equation}
 \begin{aligned}
 \mathcal G_\rho
 &:=\{T\in\GL(H):\nu_T\sim\mu\}\\
...
\begin{theorem}[Support rigidity for arbitrary bounded invertible mixing]
...
\begin{enumerate}
 ...
 \[
 \begin{array}{c|l}
 \text{scalar reflection class}&\text{allowed signed permutations}\\ \hline
...
The distinction between finite boundary loss and accumulated
infinite-dimensional loss can be stated exactly for blocks.

\begin{theorem}[Block products with possible support mismatch]
...
Nonnegative inverse operators are weighted permutations, by the
off-diagonal inverse identities used above.
...
\subsection{Boundary regularity changes the singularity threshold}
...
There is nevertheless a complete projective criterion which requires
no dichotomy.
