...
The positivity assumption in the main theorem has one precise job: it makes
finite-coordinate likelihood ratios blind to support.
...
The missing datum is row-wise support compatibility.
Each row of the operator, and for equivalence each row of the inverse, must
land in the positivity set of the one-dimensional density almost surely.

This section gives that support-aware replacement first.
Arbitrary zero sets lead to an exact zero-set row criterion; bounded
...
We then return to examples showing why the old positive-density dichotomy
fails and how boundary behavior changes the summability threshold.
...
The support-aware theorem still uses the same Fourier/Fisher mechanism for
...
The first replacement theorem is the main support-aware form of the
classification.
The Hilbert--Schmidt alternative remains the metric part of the answer.
The new condition is purely about null sets: no output coordinate may place
...
They introduce a row-by-row boundary test.
...
\cref{sec:restricted-counterexample} below satisfies the forward row
condition but fails the inverse row condition, which is exactly why it gives
...
The row test and the Hellinger energy measure different phenomena.
Boundary compatibility decides whether the transformed law is absolutely
...
metric threshold; support compatibility remains a separate row condition\\
\bottomrule
\end{tabular}
\end{center}
...
accumulation of Hellinger loss, while boundary compatibility separately
...
For bounded and one-sided supports, the row condition has much stronger
consequences than it first suggests.
Conditional expectations of finite row sums see the boundary of the support.
This turns support compatibility into an \(\ell^1\)-type row constraint, and
...
\(\alpha,\theta>0\), including the shape-two Fourier example in
\cref{sec:variance-and-affine-normalization}: only ordinary coordinate
permutations are allowed.
Compact support permits signs only to the extent allowed by the scalar
reflected law.
...
measure exactly.
A standardized asymmetric Beta law has unequal endpoint magnitudes, so it
permits only permutations.
These conclusions come from support geometry rather than from the
Fourier/Fisher estimates.

\subsection{Why the support test is indispensable}
...
implies equivalence without the inverse row condition.
